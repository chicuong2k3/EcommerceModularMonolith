...
internal sealed class ReadOrderRepository : IReadOrderRepository
{
    private readonly IDbConnection dbConnection;

    public ReadOrderRepository(IDbConnection dbConnection)
    {
    ...
    public async Task<OrderReadModel?> GetByIdAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        var order = await dbConnection.QueryFirstOrDefaultAsync<OrderReadModel>(orderQuery, new { Id = id });
        ...
        var items = await dbConnection.QueryAsync<OrderItemReadModel>(itemsQuery, new { OrderId = id });
        order.Items = items.ToList();

        return order;
    }

    public async Task<PaginationResult<OrderReadModel>> GetAllAsync(GetOrdersSpecification specification, CancellationToken cancellationToken = default)
    {
