...
internal sealed class GetProductByIdHandler(IDbConnection connection)
    : IQueryHandler<GetProductById, ProductReadModel>
{
    public async Task<Result<ProductReadModel>> Handle(GetProductById query, CancellationToken cancellationToken)
    {
        ...
        var results = await connection.QueryAsync<ProductReadModel, ProductVariantReadModel, AttributeValueReadModel, ProductReadModel>(
            sql,
